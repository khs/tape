...
def _token() -> str:
    ...
    key = os.environ.get("NOAA_API_KEY", "").strip()
    if key:
        return key
    env = HERE.parent / ".env"
    if env.exists():
        m = re.search(r"^NOAA_API_KEY=(.*)$", env.read_text(encoding="utf-8"), re.M)
        if m:
            return m.group(1).strip().strip('"').strip("'")
    raise SystemExit("NOAA_API_KEY not found in env or .env")
