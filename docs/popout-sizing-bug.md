...
- **Choropleth** (county bachelor's-degree map): map sat left-aligned at a
...
- **Choropleth** (`src/components/ChartChoropleth.astro` ~line 892): width is
  ...
  - **DONE (committed 433d43a38e, NOT verified, NOT pushed):** tile mode now
    ...
    live that the popout TILE choropleth was svg=620 in a 1352 canvas, so this
    fixes the *tile* case.
  - **REMAINING:** Keller's screenshot shows the **expanded** choropleth (1000px
...
- `433d43a38e` fix(choropleth): tile-mode fill (PARTIAL — tile only).
...
2. **Natural flow**: click a coverage chip (don't just set iframe `src` — that
   didn't trigger the choropleth expand). Let the popout open expanded.
3. Inspect the iframe's `document`: find `dialog[open]` (line chart) and the
   expanded choropleth canvas; measure widths + the cap chain (the technique
   that found the 620). Confirm which fixed width is leaving the gap.
4. Fix the **expanded** choropleth width + the **line dialog** width to fill the
